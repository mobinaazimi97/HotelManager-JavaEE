...
public class EmployeeService implements Service<Employee, Long> {
    ...
    public List<Employee> findByRoom(String room) throws Exception {
        try (CrudRepository<Employee, Long> crudRepository = new CrudRepository<>()) {
            HashMap<String , Object>params = new HashMap<>();
            params.put("rooms",room);
            return crudRepository.findBy("Employee.findByRoom",params, Employee.class);
        }
    }
}
